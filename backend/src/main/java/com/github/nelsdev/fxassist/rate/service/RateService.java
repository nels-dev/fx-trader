...
public class RateService {
  ...
  private Quote getQuote(Currency currency) {
    if (!CACHE.containsKey(currency) || CACHE.get(currency)
                                             .getSecond()
                                             .isBefore(Instant.now())) {
      var newQuote = Pair.of(quoteRepository.findByCurrency(currency), Instant.now()
                                                                              .plusSeconds(CACHE_EXPIRY_SECONDS));
